...
function drawSprite(ctx, entity, dt, entityInsideBuilding) {
    ...
    const iconWidth = icon ? ctx.measureText(icon).width + 4 : 0;
    const totalWidth = nameWidth + iconWidth;
    const startX = frameW / 2 - totalWidth / 2;

    // Draw Icon (no shadow)
    if (icon) {
        ctx.fillText(icon, startX + iconWidth / 2 - 2, -2);
    }
    
    // Set color for name
    if (entity.type === 'group' || entity.type === 'unit') {
        ctx.fillStyle = entity.color || '#fff';
    } else {
        ctx.fillStyle = '#fff'; // Default color for farmers, etc.
    }
    
    // Draw name shadow
    ctx.fillStyle = '#000';
    ctx.fillText(name, startX + iconWidth + nameWidth / 2 + 1, -1);
    ...
    ctx.fillText(name, startX + iconWidth + nameWidth / 2, -2);
