...
function step(dt){
  ...
  for(const h of allBuildings.filter(b => b.type === 'house')){
    ...
    if(t > auto.lastSpawnCheck + 1) { handleAutospawn(); auto.lastSpawnCheck = t; }
      ...
      for (const b of allBuildings) {
        ...
        if (b.doorState === 'opening') {
          ...
          b.doorAnimationTime += dt * 1000;



          if (b.doorAnimationTime >= DOOR_ANIMATION_SPEED) {
    ...
    function handleDoorProximity() {
        ...
        const proximityThreshold = 32 * 32; // Using squared distance (approx. 2 tiles)
        ...
        const activeChunks = getChunksInProximity(player.x, player.y, 2);
        ...
        const allBuildings = activeChunks.flatMap(chunk => chunk.staticEntities.filter(e => ['house', 'shop', 'kabak'].includes(e.type)));
        ...
        for (const b of allBuildings) {
            ...
            const playerDistSq = dist2(player, d);
            ...
            if (playerDistSq < proximityThreshold) {
                ...
                if (b.doorState === 'open' || b.doorState === 'opening') {
