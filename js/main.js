...
async function initWorld(startInventory = {}, spriteData = null){
  ...
  stats.kills=0; stats.startTime=performance.now(); 
  forceUpdateInventoryUI(); 
  updateDossierUI();
  ...
  for(const [key, region] of worldData.entries()) {
    ...
    for (const chunk of region.chunks.values()) {
        if (chunk.staticCache) {
            const chunkXOnMap = x + (chunk.x - rx * REGION_SIZE) * scale;
            const chunkYOnMap = y + (chunk.y - ry * REGION_SIZE) * scale;
            const chunkSizeOnMap = (CHUNK_SIZE * TILE_SIZE) * scale;
            wmctx.drawImage(chunk.staticCache, chunkXOnMap, chunkYOnMap, chunkSizeOnMap, chunkSizeOnMap);
        }
    }
...
function teleport() {
  if (player && !document.getElementById('hideCheatsChk').checked) {
    ...
    // Get chunks before updating coordinates
    const oldChunk = getChunkFromWorldCoords(prevX, prevY);
    const newChunk = getChunkFromWorldCoords(newX, newY);
    ...
    // Update chunk registration for the player group
    if (oldChunk && newChunk && oldChunk !== newChunk) {
        const index = oldChunk.dynamicEntities.findIndex(e => e.id === player.id);
        if (index > -1) {
            oldChunk.dynamicEntities.splice(index, 1);
            newChunk.dynamicEntities.push(player);
        }
    }
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setupCollapsiblePanel('graphics-settings-window');
  ...
  if (renderDistanceSlider && renderDistanceVal) {
      renderDistanceSlider.oninput = () => {
          renderDistance = parseInt(renderDistanceSlider.value, 10) || 3;
          renderDistanceVal.textContent = renderDistance;
      };
  }
  ...
  document.getElementById('chunkGridChk').onchange = e => showChunkGrid = e.target.checked;
