...
function newHouse(x,y, village=null){ 
  const h = {type:'house', x,y,w:HOUSE_SIZE,h:HOUSE_SIZE,roof:1,owner:null,ownerColor:'#777',garrison:[],id:Math.random().toString(36).slice(2,8),doorSide:2,lastFire:0,lockedBy:null, village, isMissionTarget: false}; 
  const region = getOrCreateRegion(x, y);
  const { cx, cy } = worldToChunkCoords(x, y);
  const chunk = getOrCreateChunk(region, cx, cy);
...
function newShop(x,y, village=null){
  const shop = {type:'shop', x,y,w:SHOP_SIZE,h:SHOP_SIZE,roof:1,doorSide:2,lockedBy:null,id:Math.random(), village};
  const region = getOrCreateRegion(x, y);
  const { cx, cy } = worldToChunkCoords(x, y);
  const chunk = getOrCreateChunk(region, cx, cy);
...
function newKabak(x,y, village=null){
  const kabak = {type:'kabak', x,y,w:KABAK_SIZE,h:KABAK_SIZE,roof:1,doorSide:2,lockedBy:null,id:Math.random().toString(36).slice(2,8), village, missions: []};
  const region = getOrCreateRegion(x, y);
  const { cx, cy } = worldToChunkCoords(x, y);
  const chunk = getOrCreateChunk(region, cx, cy);
